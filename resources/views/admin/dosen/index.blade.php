    ...
    <x-slot name="header">
        <div class="flex justify-between items-center">
            ...
            <div class="flex space-x-3">
                <a href="{{ route('admin.dosen.create') }}" class="bg-blue-600 hover:bg-blue-700 text-white px-4 py-2 rounded-lg shadow-sm font-medium transition">Tambah Dosen</a>
            </div>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 bg-white shadow-sm sm:rounded-xl p-6 border border-gray-100">
                    ...
                    <form action="{{ route('admin.dosen.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-3 gap-4">
                        <div class="md:col-span-2">
                            <label class="block text-xs font-bold uppercase text-gray-500 mb-1">Cari Nama / Email / NIDN</label>
                        ...
                        <div class="md:col-span-3 flex justify-end space-x-2">
                            @if($search || $prodiId)
                                <a href="{{ route('admin.dosen.index') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-gray-700 hover:bg-gray-50 text-sm font-medium transition">Reset</a>
                            @endif
                            ...
                            <a href="{{ route('admin.dosen.export', request()->query()) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-5 py-2 rounded-lg text-sm font-medium transition flex items-center">
                ...
                <div class="bg-white shadow-sm sm:rounded-xl p-6 border border-gray-100">
                    ...
                    <form action="{{ route('admin.dosen.import') }}" method="POST" enctype="multipart/form-data" class="space-y-4">
                        @csrf
                        <div>
                            <label class="block text-xs text-gray-500 mb-2">Upload file CSV dosen dengan format kolom: <br><strong class="text-gray-700 font-bold">name, email, nidn, prodi_name</strong></label>
                    ...
                    <table class="min-w-full leading-normal">
                        <thead>
                            <tr>
                                ...
                                <th class="px-5 py-4 border-b-2 border-gray-100 bg-gray-50 text-left text-xs font-bold text-gray-600 uppercase tracking-wider">NIDN</th>
                            ...
                            @foreach($dosens as $item)
                                <tr class="hover:bg-gray-50 transition-colors">
                                    ...
                                    <td class="px-5 py-4 bg-white text-sm">
                                        <div class="flex space-x-3">
                                            <a href="{{ route('admin.dosen.edit', $item->id) }}" class="text-blue-600 hover:text-blue-900 font-medium transition">Edit</a>
                                            <form action="{{ route('admin.dosen.destroy', $item->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus dosen ini?');" class="inline">
                                                @csrf
                                                @method('DELETE')
